const ZQL_multivideo = {
      ...
      ZQL_apis.detectStream().then((res) => {
        if (document.visibilityState == "visible" && res.code == 0) {
          for (let i = 0; i < ZQL_playingSource.videonum; i++) {
            let cameraId = ZQL_playingSource[i];
            if (
              cameraId &&
              ZQL_videosInfos[i] &&
              ZQL_videosInfos[i].status != "离线"
            ) {
              let cameraId = ZQL_sources[cameraId].sourceId;
              let streamInfo = res.streams.find(
                (item) => item.url.indexOf(`${cameraId}`) > 0
              );
              if (
                !streamInfo ||
                ...
                ZQL_multivideo.subscribeLive(cameraId, i);
              }
            }
          }
        }
  ...
  playVideo(cameraId, index) {
    ...
    if (stream.indexOf("webrtc") >= 0) {
      ...
      let src = `http://${ZQLGLOBAL.serverIp}:${ZQLGLOBAL.srs_http_server}/live${
        stream.split("/live")[1]
      }`;
